...
int double_hashing(long k, long n, int i) {
...
// -----------------+++++++-------------------------------/
	...
	for(int i = 0; i<n;i++){
		...
		int hash_value = double_hashing(k, (long) n, intentos);

		while(ht_hashing_cerrdado_dh2[hash_value] != -1 && intentos < n){
			intentos += 1;
			hash_value = double_hashing(k, (long) n, intentos);
		}
